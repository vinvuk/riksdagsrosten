...
export default function AmnePage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
      <PageHeader
        title="Ämnen"
        subtitle="Riksdagens voteringar indelade efter utskottsområde. Välj ett ämne för att se alla voteringar inom området."
      />
        ...
        {Object.entries(COMMITTEE_MAP).map(([code, info]) => {
          ...
          return (
            <Link
              key={code}
              href={`/amne/${info.slug}`}
              className="group p-6 rounded-lg bg-base-100 ring-1 ring-base-300 shadow-sm hover:shadow-md hover:bg-base-200 transition-all"
            >
